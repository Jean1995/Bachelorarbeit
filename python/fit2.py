...
from params import m_p, m_0, m_b, m_d, N1, N2
import params
...
for i, val in enumerate(x_plot):
    ...
    for k in range(N1):
        mc_values_p = mc_values_p + a_mc.T[k]*f(val, k, m_p) # bestimme die Werte für f+, f0 für das jeweilige q^2
    for k in range(N2):
        mc_values_n = mc_values_n + a_mc.T[k+N1]*f(val, k, m_0)
