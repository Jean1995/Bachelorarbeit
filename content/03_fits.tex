...
Dieser quantiative Unterschied lässt sich dadurch begründen, dass in der vorliegenden Quelle eine andere Parametrisierung der Formfaktoren verwendet wird.
...
\end{equation}
Dies entspricht einer Abweichung vom errechneten Theoriewert für $N_+=\num{3}$ und $N_0=\num{3}$ in Tabelle \ref{tab:r_calc} von $\input{pycode/R_abweichung_33.tex}\sigma$.
...
\begin{align*}
  \hat{A}_S &= \input{pycode/As.tex}, \\
  \hat{A}_{VS} &= \input{pycode/Avs.tex}
\end{align*}
...
Es ist zu beachten, dass die Kurven mit den Eigenschaften $\mathrm{Re}(C_{S1}) = \input{pycode/Cs1_Re.tex}$, $\mathrm{Im}(C_{S1}) = \pm\input{pycode/Cs1_Im.tex}$ zusammenfallen, da sich die $C_{S1}$-Werte lediglich im Vorzeichen des Realteils unterscheiden, der Realteil jedoch ausschließlich betragsmäßig in die differentielle Zerfallsbreite eingeht.
Zusätzlich fällt auf, dass durch der Wahl der Position von $C_{S1}$ auf der komplexen Ebene ausgewählt wird, für welchen Impulsübertrag die maximale differentielle Zerfallsbreite der Tauonen zu erwarten ist.
Eine Messung der Position des Maximums lässt dementsprechend Rückschlüsse auf mögliche Werte von $C_{S1}$ zu.
Bei der Betrachtung der Korrekturen der Formfaktoren innerhalb des Standardmodelles, siehe Kapitel \ref{sec:naiv}, hat sich das Maximum der Verteilung für die hier betrachteten Parameterpaare ($\alpha,\: \beta$) nur geringfügig verändert.
Somit würde eine große Abweichung des Maximums vom zu erwartenden Theoriewert einen Hinweis darauf geben, dass eine Korrektur außerhalb des Standardmodelles, beispielsweise durch die Einführung von $C_{S1}$, zu erwarten ist.
