...
Der Wilson-Koeffizient $C_{S_1}^\tau$ ist eine im Allgemeinen komplexe Größe und beschreibt somit die Stärke des Einflusses der NP-Kopplung.
Ziel ist es, den Koeffizienten $C_{S_1}^\tau$ so anzupassen, dass die theoretische Vorhersage für $R$ mit den experimentellen Werten aus Gleichung \eqref{eqn:R_exp} übereinstimmen.
...
\begin{equation}
  \frac{\mathrm{d} \Gamma}{\mathrm{d} q^2} \left(\overline{B} \to D \tau \overline{\nu_{\tau}} \right) = \alpha \left( c_+^{\tau} f_+(q^2)^2 + \left(c_0^{\tau} + \beta_1 \lvert C_{S_1}^{\tau} \rvert^2 + \beta_2 \operatorname{Re}(C_{S_1}^{\tau})\right) f_0(q^2)^2 \right)
\end{equation}
mit den Abkürzungen
\begin{align*}
  \beta_1 &= \frac{3}{2} ( \frac{m_B^2 - m_D^2}{m_b - m_c} )^2 \frac{q^2}{m_B^4} & \beta_2 &= 3  \frac{m_\tau}{m_B^4} \frac{(m_B^2 - m_D^2)^2}{m_b - m_c}
\end{align*}
und
\begin{align*}
...
Integriert über den kinematisch erlaubten Bereich ergibt sich die abgekürzte Form
\begin{equation}
  {\mathbfcal{B}}\!\left(\overline{B} \to D \tau \overline{\nu_\tau} \right) = {\mathbfcal{B}}_\text{SM}\!\left(\overline{B} \to D \tau \overline{\nu_\tau} \right) + A_S \lvert C_{S_1}^{\tau} \rvert^2 + A_{VS} \operatorname{Re}(C_{S_1}^{\tau}).
\end{equation}
Diese Parameter lassen sich unter Nutzung der Fits für die Ordnungen $N_+ = N_0 = \num{3}$ zu
\begin{align*}
