...
Für $\alpha_\text{s} > 1$ steigt dementsprechend der Einfluss von Diagrammen nicht-führender Ordnung $n>1$, sodass eine Entwicklung in $\alpha_\text{s}$ als Störungstheorie nicht mehr möglich ist.
...
\end{equation}
dar, dessen Abhängigkeit von $q^2$ in Abbildung \ref{fig:w_param} grafisch dargestellt ist.
...
\begin{figure}
  ...
  \caption{Bildmenge der Abbildung $z(q^2)$ in der komplexe Ebene. In Grün wird der nach Gleichung \eqref{eqn:kinematik} kinematisch erlaubte Bereich dargestellt. Die gestrichelte rote Linie verdeutlicht den unphysikalischen Bereich negativer Impulsüberträge.}
  \label{fig:z_kreis}
\end{figure}
